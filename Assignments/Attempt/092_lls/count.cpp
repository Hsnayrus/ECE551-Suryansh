...
template <typename T> class Counted_OrdItem {
  public:
    T item;
    unsigned int count;
...
template <typename T> class Counted_OrdCount {
  public:
    T item;
    unsigned int count;
    Counted_OrdCount() : item(), count(0) {}
    Counted_OrdCount(const Counted_OrdItem<T> &rhs)
        : item(rhs.item), count(rhs.count) {}
    bool operator<(const Counted_OrdCount &rhs) const {
        return count < rhs.count;
    }
};

template <typename T> void sort(LinkedList<T> &ll) {
    bool sorted = false;
    int sz = ll.getSize();
    while (!sorted) {
        sorted = true;
...
template <typename T>
LinkedList<Counted_OrdCount<T> >
convertTypes(LinkedList<Counted_OrdItem<T> > ll) {
...
int isidchar(char c) { return isalnum(c) || c == '_'; }
